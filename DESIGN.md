...
- Helvetica Neue as the universal display + body face — no custom font loading required
...
### Font Family
**Helvetica Neue** (with fallback: Helvetica → Arial → sans-serif) is the primary typeface. No custom font import needed — Helvetica Neue is a system font on macOS/iOS, and Arial is the Windows equivalent. Do not use `font-feature-settings` as Helvetica does not have OpenType stylistic sets.

### Hierarchy

| Token | Size | Weight | Line Height | Letter Spacing | Use |
|---|---|---|---|---|---|
| `{typography.hero-display}` | 64px | 700 | 1.10 | -0.5px | Homepage hero |
| `{typography.display-lg}` | 48px | 700 | 1.12 | -0.3px | Section-opener display |
| `{typography.heading-lg}` | 36px | 700 | 1.20 | -0.2px | Subsection headlines |
| `{typography.heading-md}` | 28px | 600 | 1.25 | -0.1px | Editorial subheads |
| `{typography.heading-sm}` | 24px | 600 | 1.30 | 0 | Card titles, feature-tile headers |
...
### Principles
- Negative letter-spacing on body roles (`-0.14px` to `-0.16px`) tightens the type fractionally for a clean, modern look.
- Use `font-weight: 700` for hero and heading roles; `600` for sub-headings; `400` for body — this creates a clear three-tier visual rhythm.
- Avoid `font-feature-settings` — Helvetica Neue does not have OpenType alternates like Optimistic VF.
...
- Use `font-weight: 700` for hero/display and `600` for sub-headings — do not use `font-feature-settings` with Helvetica.
- Use the 600-weight `{typography.heading-md}` for editorial subheads — it creates a clear visual rhythm against the 700-weight displays.
